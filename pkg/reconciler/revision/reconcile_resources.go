...
func (c *Reconciler) reconcilePA(ctx context.Context, rev *v1.Revision) error {
	...
	if isReservedRoutingState(rev) && isRevisionStateFinal(rev) && isPAStateScaledDown(pa) {
		logger.Info("Revision is not receiving traffic: ", rev)
...
func isPAStateScaledDown(pa *v1alpha1.PodAutoscaler) bool {
	return pa.Status.ActualScale != nil && *pa.Status.ActualScale == int32(0)
}
